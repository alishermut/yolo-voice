...
pub struct TDTModelConfig {
    ...
    pub initial_token_id: Option<usize>,
    pub suppressed_token_ids: Vec<usize>,
}
...
impl TDTModelConfig {
    ...
    /// Seed the prediction network with a known context token.
    pub fn with_initial_token_id(mut self, initial_token_id: Option<usize>) -> Self {
        self.initial_token_id = initial_token_id;
        self
    }

    /// Prevent selected control tokens from changing decoder context.
    pub fn with_suppressed_token_ids(mut self, suppressed_token_ids: Vec<usize>) -> Self {
...
impl ParakeetTDTModel {
    ...
    pub fn from_pretrained<P: AsRef<Path>>(
        ...
        let config = TDTModelConfig::new(vocab_size)
            .with_initial_token_id(initial_token_id)
            .with_suppressed_token_ids(suppressed_token_ids);
    ...
    fn greedy_decode(
        ...
        let mut last_emitted_token = self
            .config
            .initial_token_id
        ...
        while t < time_steps {
            ...
            let token_id = vocab_logits
                .iter()
                .enumerate()
                .filter(|(idx, _)| !self.config.suppressed_token_ids.contains(idx))
