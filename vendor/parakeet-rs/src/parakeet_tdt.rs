...
impl ParakeetTDT {
    ...
    pub fn from_pretrained<P: AsRef<Path>>(
        ...
        let initial_token_id = vocab.token_to_id("<|en|>");
        let suppressed_token_ids = match (
            vocab.token_to_id("<|unklang|>"),
            vocab.token_to_id("<|zu|>"),
        ) {
            (Some(start), Some(end)) if start <= end => (start..=end).collect(),
            _ => Vec::new(),
        };
        let model = ParakeetTDTModel::from_pretrained(
            path,
            exec_config,
            vocab_size,
            initial_token_id,
